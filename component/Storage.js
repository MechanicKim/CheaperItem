...
export const getItems = () => {
  ...
  return realm.objects('ItemPrice').filtered('TRUEPREDICATE SORT(item ASC, price ASC) DISTINCT(item)');
}
...
export const saveItem = (item, callback) => {
  ...
  realm.write(() => {
    realm.create('ItemPrice', {
      ...
      unitValue: parseInt(item.unitValue || 0),
      unit: item.unit
    });
...
export const updateItem = (item, callback) => {
  const realm = new Realm();
  realm.write(() => {
    const storedItem = realm.objects('ItemPrice').filtered(`id='${item.id}'`)[0];
    storedItem.mart = item.mart;
    storedItem.item = item.name;
    storedItem.price = parseInt(item.price);
    storedItem.unitValue = parseInt(item.unitValue || 0);
...
export const searchItem = keyword => {
  const realm = new Realm();
  return getItems().filtered(`item CONTAINS[c] "${keyword}"`).sorted('item').sorted('price');
}

export const groupBy = query => {
  const realm = new Realm();
  return realm.objects('ItemPrice').filtered(`TRUEPREDICATE DISTINCT(${query})`);
}
